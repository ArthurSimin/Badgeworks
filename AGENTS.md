...
- Server-side text layout measures Inter exactly with `opentype.js` + the bundled TTFs in `assets/fonts/` (`fontMeasureText` in `badge-core.js`), so API-badge widths match the browser app's canvas measurement. The browser app injects its own canvas `measureText`; `approxMeasureText` remains only as a fallback when the fonts/opentype.js are unavailable.
- Text rendering is fully self-contained: `buildBadgeSvg` embeds Inter as `@font-face` data-URIs in the SVG `<defs>` when `opts.embedFonts` is true (server passes it). sharp/librsvg then renders exact Inter with no system-font installation. `server.js` strips the `@font-face` block from the SVG in the JSON response so Discord code blocks stay small — the PNG is always rasterized from the font-embedded SVG.
