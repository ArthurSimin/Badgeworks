/* ==========================================================================
   BADGEWORKS CORE
   Pure badge-rendering engine shared by the browser app (app.js) and the
   Node API server (server.js). No DOM, no canvas, no fetch — the caller
   injects a `measureText(text, fontSpec)` function for text layout.
   ========================================================================== */
...
// FontAwesome pack tokens for class parsing (shared with the browser app)
...
// Default configuration - mirrors the website's initial UI state so a partial
// config (e.g. from API query params) renders exactly like the default editor.
const DEFAULT_CONFIG = {
  ...
  faPack: 'fa-brands',
  faIconName: 'github',
  faIconInput: 'fa-brands fa-github',
  showDisk: false,
  diskColor: '#ffffff',
  logoColor: '#0f141c',
...
function parseFaClass(iconClass) {
  if (!iconClass) return { style: 'solid', packFull: 'fa-solid', name: null };
...
// Fill in every missing field from DEFAULT_CONFIG. Unknown keys are ignored.
function normalizeConfig(input) {
  const src = (input && typeof input === 'object') ? input : {};
  const cfg = {};
  for (const key of Object.keys(DEFAULT_CONFIG)) {
    const v = src[key];
    cfg[key] = (v === undefined || v === null) ? DEFAULT_CONFIG[key] : v;
  }
  if (!Array.isArray(cfg.bgStops) || cfg.bgStops.length < 2) {
    cfg.bgStops = DEFAULT_CONFIG.bgStops.slice();
  } else {
    cfg.bgStops = cfg.bgStops.slice(0, BG_GRADIENT_MAX_STOPS);
  }
  if (cfg.bgGradPreset && cfg.bgGradPreset !== 'custom' && BG_GRADIENT_PRESETS[cfg.bgGradPreset]) {
    cfg.bgStops = BG_GRADIENT_PRESETS[cfg.bgGradPreset].slice();
  }
  return cfg;
}

// Decode a data:image/svg+xml;... URL or raw <svg> string into inner path markup
function parseSvgPathContent(svgStr) {
  if (!svgStr) return '';
  let str = String(svgStr);
  ...
  const m = str.match(/<svg[^>]*>([\s\S]*?)<\/svg>/i);
  return m ? m[1].trim() : str;
}

// Export-ready measurement fallback (used by Node when no canvas exists).
// Heuristic tuned to Inter; the browser app injects exact canvas measurement instead.
function approxMeasureText(text, fontSpec) {
  if (!text) return 0;
  ...
  for (const ch of String(text)) {
    ...
    else if (ch >= '\u0391' && ch <= '\u03C9') w += 0.62;
    else if (ch >= '\uAC00' && ch <= '\uD7A3') w += 1.0;
    else if (ch >= '\u4E00' && ch <= '\u9FFF') w += 1.0;
    else if (ch >= '\u1F300' && ch <= '\u1FAFF') w += 1.0;
    else w += per;
  }
...
//   faIcon      - resolved FontAwesome icon { viewBox, pathData } or null
//   measureText - (text, fontSpec) => width in px; defaults to approxMeasureText
//   opts.faSlot - when true and FA mode has no resolved icon, emit `<!-- FA_ICON_SLOT -->`
//                 and return faPlaceholder geometry (live-preview async swap).
// Returns { svg, width, height, faPlaceholder }.
function buildBadgeSvg(rawCfg, styleName, faIcon, measureText, opts) {
  ...
  const bottomText = cfg.bottomText;
  const showDisk = !!cfg.showDisk;
  const diskColor = cfg.diskColor;
  const logoColor = cfg.logoColor;
  const bgStops = cfg.bgStops;
  const textColor = cfg.textColor;
  const radius = parseInt(cfg.radius, 10) || 0;
  const paddingRight = parseInt(cfg.paddingRight, 10) || 8;

  const diskDiameter = parseInt(cfg.diskDiameter, 10) || 40;
  const userLogoScale = parseInt(cfg.userLogoScale, 10) || 34;
  ...
  if (st === 'compact') {
    ...
    isSingleLine = true;
  } else if (st === 'cozy-minimal') {
    height = 64;
    isMinimal = true;
  } else if (st === 'compact-minimal') {
    height = 40;
    isMinimal = true;
  }
  ...
  const noLogo = cfg.logoPosition === 'none';
  const logoOnRight = cfg.logoPosition === 'right';
  ...
  const presetKey = cfg.presetKey;
  const brandInfo = OFFICIAL_BRAND_ICONS[presetKey] || OFFICIAL_BRAND_ICONS.github;

  const isCustomArtboard = cfg.iconMode === 'preset' && brandInfo.isCustomSvg && !brandInfo.scalable;
  ...
  if (!noLogo) {
    if (logoOnRight) {
      textX = leftPad;
    } else if (brandInfo && brandInfo.textX && cfg.iconMode === 'preset') {
  ...
  if (!isMinimal) {
    ...
    if (isSingleLine) {
      maxTextW = bottomText ? measure(bottomText, bottomFont) * textCorrectionFactor : 0;
    } else {
      const topW = topText ? measure(topText, topFont) * textCorrectionFactor : 0;
      const bottomW = bottomText ? measure(bottomText, bottomFont) * textCorrectionFactor : 0;
    ...
    if (noLogo) {
      ...
      let textXLeft;
      if (brandInfo && brandInfo.textX && cfg.iconMode === 'preset') {
  ...
  const useTextGrad = !!cfg.useTextGrad;
  const textGradTop = cfg.textGradTop;
  const textGradBot = cfg.textGradBot;
  const useTextStroke = !!cfg.useTextStroke;
  const textStrokeColor = cfg.textStrokeColor;
  const textStrokeWidth = cfg.textStrokeWidth;
  const subtitleColor = cfg.subtitleColor;
  const useTextShadow = !!cfg.useTextShadow;
  const textShadowColor = cfg.textShadowColor;
  const textShadowBlur = parseFloat(cfg.textShadowBlur) || 0;

  const useCustomLogoColor = !!cfg.useCustomLogoColor;
  const customLogoColor = cfg.logoColor;
  const useLogoStroke = !!cfg.useLogoStroke;
  const logoStrokeColor = cfg.logoStrokeColor;
  const logoStrokeWidth = cfg.logoStrokeWidth;
  const useLogoShadow = !!cfg.useLogoShadow;
  const logoShadowColor = cfg.logoShadowColor;
  const logoShadowBlur = parseFloat(cfg.logoShadowBlur) || 0;
  ...
  svgMarkup += '  <defs>\n';
  svgMarkup += '    <linearGradient id="badge-bg-' + st + '" x1="0" y1="0" x2="0" y2="' + height + '" gradientUnits="userSpaceOnUse">\n';
  bgStops.forEach(function (hex, i) {
    svgMarkup += '      <stop offset="' + (i / (bgStops.length - 1)).toFixed(3) + '" stop-color="' + hex + '"/>\n';
  });
  svgMarkup += '    </linearGradient>\n';

  if (useTextGrad) {
    svgMarkup += '    <linearGradient id="badge-text-grad-' + st + '" x1="0" y1="0" x2="0" y2="100%" gradientUnits="userSpaceOnUse">\n';
    svgMarkup += '      <stop stop-color="' + textGradTop + '"/>\n';
    svgMarkup += '      <stop offset="1" stop-color="' + textGradBot + '"/>\n';
    svgMarkup += '    </linearGradient>\n';
  }

  svgMarkup += buildLogoFxFilter('badge-logo-fx-' + st, {
  ...
  svgMarkup += buildLogoFxFilter('badge-text-fx-' + st, {
    useTint: false,
    useStroke: false,
  ...
  svgMarkup += '  <rect width="' + (width - 2) + '" height="' + (height - 2) + '" x="1" y="1" stroke="#ffffff" stroke-opacity=".15" stroke-width="2" rx="' + Math.max(0, radius - 1) + '"/>\n';

  const useLogoFxFilter = useCustomLogoColor || useLogoStroke || useLogoShadow;
  const logoFilterAttr = useLogoFxFilter ? ' filter="url(#badge-logo-fx-' + st + ')"' : '';
  const textShadowAttr = useTextShadow ? ' filter="url(#badge-text-fx-' + st + ')"' : '';
  ...
  let faPlaceholder = null;
  if (!noLogo) {
    if (cfg.iconMode === 'preset') {
    ...
    } else if (cfg.iconMode === 'upload' && cfg.uploadedDataUrl) {
      const imgSize = effectiveLogoSize;
      const imgY = Math.round((height - imgSize) / 2);
      svgMarkup += '  <!-- Uploaded Image/SVG Logo -->\n';
      svgMarkup += '  <image href="' + cfg.uploadedDataUrl + '" xlink:href="' + cfg.uploadedDataUrl + '" x="' + iconX + '" y="' + imgY + '" width="' + imgSize + '" height="' + imgSize + '" preserveAspectRatio="xMidYMid fit"' + logoFilterAttr + '/>\n';
    } else if (cfg.iconMode === 'fontawesome') {
      const imgSize = effectiveLogoSize;
      const imgY = Math.round((height - imgSize) / 2);
      const fillColor = showDisk ? logoColor : (useCustomLogoColor ? customLogoColor : textColor);
      if (faIcon && faIcon.pathData) {
        svgMarkup += '  <!-- FontAwesome Icon -->\n';
        svgMarkup += '  <svg x="' + iconX + '" y="' + imgY + '" width="' + imgSize + '" height="' + imgSize + '" viewBox="' + faIcon.viewBox + '" fill="' + fillColor + '" xmlns="http://www.w3.org/2000/svg"><path d="' + faIcon.pathData + '"/></svg>\n';
      } else if (opts.faSlot) {
        svgMarkup += '  <!-- FA_ICON_SLOT -->\n';
        faPlaceholder = { x: iconX, y: imgY, size: imgSize, fill: fillColor, height: height, width: width, effectiveLogoSize: effectiveLogoSize };
      } else {
        svgMarkup += '  <!-- FontAwesome Placeholder -->\n';
        svgMarkup += '  <text x="' + Math.round(iconX + imgSize / 2) + '" y="' + Math.round(height / 2) + '" fill="#ffffff" font-size="' + effectiveLogoSize + '" font-family="\'Dosis\', \'Inter\', sans-serif" font-weight="600" text-anchor="middle" dominant-baseline="central" dy="-0.05em">?</text>\n';
      }
    } else if (cfg.iconMode === 'raw') {
      const rawContent = cfg.customSvgContent || parseSvgPathContent(cfg.rawSvgDataUrl);
      if (rawContent) {
        const imgSize = effectiveLogoSize;
        const imgY = Math.round((height - imgSize) / 2);
        const rawHref = cfg.rawSvgDataUrl || ('data:image/svg+xml;utf8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg">' + rawContent + '</svg>'));
        svgMarkup += '  <!-- Raw Pasted SVG Logo -->\n';
        svgMarkup += '  <image href="' + rawHref + '" xlink:href="' + rawHref + '" x="' + iconX + '" y="' + imgY + '" width="' + imgSize + '" height="' + imgSize + '" preserveAspectRatio="xMidYMid fit"' + logoFilterAttr + '/>\n';
      }
    }
  }

  const textFillAttr = useTextGrad ? 'fill="url(#badge-text-grad-' + st + ')"' : 'fill="' + textColor + '"';
  ...
  if (!isMinimal) {
    if (isSingleLine) {
      const compactTextW = bottomText ? measure(bottomText, bottomFont) * 0.98 : 0;
      ...
      const topMeasured = topText ? measure(topText, topFont) * 0.98 : 0;
      const bottomMeasured = bottomText ? measure(bottomText, bottomFont) * 0.98 : 0;
      if (topText) {
  ...
  return { svg: svgMarkup, width: width, height: height, faPlaceholder: faPlaceholder };
}

return {
  OFFICIAL_BRAND_ICONS: OFFICIAL_BRAND_ICONS,
  FA_PACK_TOKENS: FA_PACK_TOKENS,
  BG_GRADIENT_PRESETS: BG_GRADIENT_PRESETS,
  BG_GRADIENT_MAX_STOPS: BG_GRADIENT_MAX_STOPS,
  DEFAULT_CONFIG: DEFAULT_CONFIG,
  parseFaClass: parseFaClass,
  escapeHtml: escapeHtml,
  buildLogoFxFilter: buildLogoFxFilter,
  parseSvgPathContent: parseSvgPathContent,
  normalizeConfig: normalizeConfig,
  approxMeasureText: approxMeasureText,
  buildBadgeSvg: buildBadgeSvg
};
});
